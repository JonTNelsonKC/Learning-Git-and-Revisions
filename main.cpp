...
int main()
{
    ...
    cin >> playAgain;
    switch (playAgain)
    { // switch statement that lets the user opt in or out of the multiplaction tutor
    case 'y':
        cout << "Let's see how good you are, " << name << endl;

        do
        { // runs the game until the user enters "no"
            for (i = 1; i <= 12; i = i + 1)
            {                                    // runs the following block of code 12 times
                ...
                { // prints correct and keeps track of the user's score
                    cout << "Correct!" << endl;
                    score = score + 1;
                }
                else
                { // prints if the user enters a wrong answer
